...
export default class SearchStore implements ILocalStore {
  ...
  private _selectedCategories: Collection<ProductCategory['id'], ProductCategory> =
  ...
  private _filterSelectedCategories(categories: ProductCategory[]): void {
    const filtredCategories = categories.filter((item) =>
      this._selectedCategories.order.includes(item.id)
    );
  ...
  private _getSelectedCategoriesByIds(categories: ProductCategory[]): void {
    const filtredCategories = categories.filter((item) =>
      this._pendingSelectedIds?.includes(item.id)
    );
    this.setSelectedCategories(filtredCategories);
  ...
  private setSelectedCategories(categories: ProductCategory[]): void {
    this._selectedCategories = normalizeCollection(categories, (item) => item.id);
  }
  ...
  get title(): string {
    if (this.selectedCategories.length > 0) {
      return this.selectedCategories.map((item) => item.title).join(', ');
    }
